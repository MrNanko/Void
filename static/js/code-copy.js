document.addEventListener('DOMContentLoaded', function() {
    // Add copy button only for code blocks without line numbers
    const codeBlocks = document.querySelectorAll('pre code');
    
    codeBlocks.forEach(function(codeBlock) {
        const pre = codeBlock.parentElement;
        
        // Check if it's a line number column code block - skip if it is
        ...
        copyButton.addEventListener('click', function() {
            // Get code text, remove line numbers
            let codeText = '';
            const spans = codeBlock.querySelectorAll('span[style*="display:flex"]');
            
            if (spans.length > 0) {
                // Handle code wrapped with display:flex
                spans.forEach(span => {
